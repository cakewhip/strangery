...
public class StrangeMonumentPiece extends SimpleStructurePiece {
    ...
    public StrangeMonumentPiece(StructureManager structureManager, CompoundTag compoundTag) {
        super(Strangery.WF.STRANGE_MONUMENT_PIECE, compoundTag);
    }

    public StrangeMonumentPiece(StructureManager structureManager, BlockPos blockPos) {
    ...
    public boolean generate(StructureWorldAccess world,
                            ...
                            Random random, BlockBox boundingBox, ChunkPos chunkPos,
                            BlockPos blockPos) {
        if (!generated) {
            generated = true;
            ...
            Set<BlockPos> spherePosSet = GenUtil.getSphereBlockOffsets(radius)
                .stream()
                .map(spherePos -> originPos.add(spherePos.getX(), spherePos.getY(), spherePos.getZ()))
    ...
    private void genOre(StructureWorldAccess world, BlockPos blockPos, Random random,
                        Set<BlockPos> sphereBlocks) {
        BlockState blockState = random.nextBoolean() ? Strangery.B.MOONSTONE_ORE.getDefaultState() :
    ...
    @Override
    protected void handleMetadata(String metadata, BlockPos pos,
                                  ServerWorldAccess serverWorldAccess, Random random,
