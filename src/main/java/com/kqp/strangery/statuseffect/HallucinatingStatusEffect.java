...
public class HallucinatingStatusEffect extends CustomStatusEffect {
    ...
    static {
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_CREEPER_PRIMED, false)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_CREEPER_PRIMED, false)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_CREEPER_HURT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_ZOMBIE_AMBIENT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_SKELETON_AMBIENT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_SPIDER_AMBIENT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_ENDERMAN_AMBIENT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_ENDERMAN_TELEPORT)
        );
        HALLUCINATION_SOUNDS.add(
            new HallucinationSound(SoundEvents.ENTITY_STRAY_AMBIENT)
        );
    }
    ...
    public void applyUpdateEffect(LivingEntity entity, int amplifier) {
        HallucinationSound hallucinationSound = HALLUCINATION_SOUNDS.get(
            RANDOM.nextInt(HALLUCINATION_SOUNDS.size())
        );
        entity.playSound(
    ...
    public boolean canApplyUpdateEffect(int duration, int amplifier) {
        return (
            duration % 20 == 0 &&
            RANDOM.nextDouble() <
            (HALLUCINATION_CHANCE + amplifier * AMPLIFY_CHANCE) &&
            duration > 0
        );
    }
